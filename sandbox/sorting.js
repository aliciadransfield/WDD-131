...
const lowerList = simpleList.map((item) => item.toLowerCase);
...
function searchList(list, q) {
    function searchCallback(obj) {
        ...
        return obj.name.toLowerCase().includes(q.toLowerCase()) ||
        obj.description.toLowerCase().includes(q.toLowerCase()) ||
        obj.tags.find((string) => string.toLowerCase().includes(q.toLowerCase()));
    }
...
console.log(searchList(hikes, "od"));